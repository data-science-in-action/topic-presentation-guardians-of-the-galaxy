...
&ensp; &ensp;&ensp;&ensp;<font size=6>新型冠状病毒的蔓延与扩散深深牵动着各界爱国人士的心。为了解新型冠状病毒的感染特征，预防和减小人民群众被感染的风险，我们对深圳市的新冠肺炎感染特征做了初步分析。关于疫情感染特征的初步分析主要使用的是深圳市“新型肺炎”-每日新增确诊病例个案详情数据表。
...
&ensp; &ensp;&ensp;&ensp;<font size=6>这部分从确诊病例性别、年龄、确诊趋势对深圳市疫情传播情况进行分析。</font>
...
&ensp; &ensp;&ensp;&ensp;<font size=6>截止3月5日，深圳市累计确诊病例共达418人，其中，男性感染者有219名，共占比52%；女性受访者有199名，占比48%，可以看出此次疫情感染在性别上并无明显差异。</font>
...
&ensp; &ensp;&ensp;&ensp;<font size=6>由上图可知在本次疫情中，深圳市确诊病例在30-70年龄段人数最多，患者年龄最大的为86岁，最小的为1岁，平均年龄为45岁，整体分布呈现轻微左偏，较平缓。</font>
...
```{r,echo = F }
h <- read.table(file="D:/datagender/time1.csv",sep = ",",header = T)
library(zoo)
library(xts)
...
&ensp; &ensp;&ensp;&ensp;<font size=6>自1月19日起，深圳市陆续出现确诊感染病例，疫情在1月27日至2月8日达到了爆发期。2月8日后，感染人数增速放缓，深圳市疫情出现拐点。</font>
---
## 确诊人数与出院人数
```{r,echo = F }
ch <- read.table(file="D:/datagender/chuyuan.csv",sep = ",",header = T)
library(zoo)
library(xts)
options(warn=-1)
...
&ensp; &ensp;&ensp;&ensp;<font size=6>2月3日起，深圳市新增确诊人数逐日减少。2月1日起，新增出院人数逐日增加；自2月26日起，深圳市疫情已得到基本控制，新增确诊人数逐渐趋于0。</font>


---
## 2.2二元Logistic回归

&ensp; &ensp;&ensp;&ensp;<font size=6>这一部分通过二元Logistic回归，选择确诊病例的年龄，性别，感染源，居住地是否为湖北省，发病时段这5个指标作为自变量，以发病者病况轻重（轻症患者，重症患者）为因变量，探寻影响患者病症轻重的关键因素。</font>

### 2.2.1 初始变量的选择

&ensp; &ensp;&ensp;&ensp;<font size=6>我们选取确诊病例的年龄、性别、感染源、居住地以及发病时段作为回归模型的初始自变量。给各变量分别命名，自变量：年龄$x_1$，性别$x_2$（0：男，1：女）、感染源$x_3$（0：有湖北接触史，1：无湖北接触史）、居住地是否为湖北省$x_4$（0：否，1：是）、发病时段$x_5$（0：初代感染，1：次代感染），因变量:感染者病况$Y_1$（0：轻症，1：重症）。</font>
...
&ensp; &ensp;&ensp;&ensp;<font size=6>可以看出age（年龄）和time（发病时段）是显著的，通过模型验证的，其他变量不合格，我们需要丢弃。</font>
---
## 修改后的回归方程
```{r,echo = F }
Data <- read.csv("logistic.csv")
...
&ensp; &ensp;&ensp;&ensp;<font size=6>当剔除了3个不显著的变量之后，剩余两个自变量的p值均小于显著性水平0.001，此时结果有意义，得到的模型如下：</font>
...
&ensp; &ensp;&ensp;&ensp;<font size=6>对该模型的说明解释：
  ...
  （2）$x_5$（发病时段）：在年龄（$x_1$）不变的情况下，早期感染的病例表现为重症的可能性更高。在疫情爆发早期，一是人们对病因不明，多数患者及医生仅当做普通肺炎治疗，没有及时有效的治疗手段，错过了最佳治疗时机，导致了病情的加重，二是初期医疗救治条件不能满足短期大量危重病人的需求，导致病情不能得到有效控制急剧恶化。</font>
  
    

# 3.可视化方法1
---
$ensp$ensp这部分通过绘制1月30日到3月5日的深圳市整体每日累计确诊人数变化时序图、深圳市各行政区截止3月5日累计确诊环形图，分析深圳市疫情的整体发展态势（整体增幅、拐点、最低点）及分布状况。
...
```
$emsp$emsp通过观察深圳市各区截至3月5日累计确诊人数变化折线图，可以看到，从1月30日到2月15日，各区累计确诊人数整体处于上升的趋势，从到2月15日开始，深圳各区的累计确诊人数变化已经基本稳定，说明疫情控制取得了一定的效果。
---
...
$ensp$ensp通过深圳各区累计确诊人数占比环形图，可以很清楚地看到深圳南山区、福田区是确诊人数占比最大的区，分别占比0.221和0.208，这很可能与深圳这两个区的人流量大等因素有关系，而大棚区和盐田区占比最少，分别占0.015和0.005。
...
$emsp$emsp这部分主要介绍如何使用R语言实现疫情地图的可视化。疫情地图的可视化可以更加直观地反映疫情在不同地区的分布情况以及严重程度，我们在静态疫情地图的基础上，进一步绘制出可以反映疫情在时间、空间两个维度上传播的动态地图动态疫情地图。
---
### 4.1.1静态疫情地图绘制过程
...
---
### 4.3结论
$emsp$emsp通过观察深圳市动态疫情地图，我们可以发现在1月30日至2月8日之间，深圳市各行政区疫情患者日增速较快,自2月20日后，各区累计确诊数变动不大，增速明显放缓，说明疫情得到了有效控制。

$emsp$emsp此外，从时间空间传播角度，还可以发现疫情有以南山区、福田区为中心随时间向周围行政区扩散的趋势，并且深圳市疫情呈现明显两极分化的趋势，西部各区为重灾区，而东部三区累计确诊病例数均停留在个位数上。

$emsp$emsp结合深圳市各区经济发展等因素进行粗略分析，发现其疫情与各区人口密度，交通便利度、区位功能重要性、地理环境等密切相关，比如东部三区以山地为主，经济欠发达，而西部区域分别建有地铁线、高铁站、机场等大型交通枢纽，还包括数家世界500强企业,人口流量大，活动范围广，因此，疫情在东西部的传播范围呈现出显著差异。

$emsp$emsp在初步分析的过程中，疫情地图可视化可以帮助我们拓展研究思路，挖掘更多隐藏信息，为深入分析疫情的时空传播模式和特征提供方向。
